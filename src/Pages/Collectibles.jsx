...
import { X, SearchCode, ShieldCheck, ArrowLeft, ArrowRight, ChevronDown } from 'lucide-react';

// Premium curated Collectibles Database aligned with the exact menu categories
const COLLECTIBLES_ITEMS = [
  { 
    id: 1, 
    title: "1907 Saint-Gaudens Double Eagle $20 Coin", 
    origin: "U.S. Mint, Philadelphia", 
    category: "coins-money-stamps", 
    image: "https://images.unsplash.com/photo-1621972750749-0fbb1abb7736?w=600", 
    images: ["https://images.unsplash.com/photo-1621972750749-0fbb1abb7736?w=600"], 
    basePrice: 15000, 
    currentBid: 16400, 
    bidsCount: 22, 
    estimate: "$18,000 - $25,000", 
    description: "An exceptionally preserved high-relief gold double eagle. Extremely rare variant featuring Roman numerals and stunning original luster." 
  },
  { 
    id: 2, 
    title: "19th Century Brass Astrolabe & Celestial Calculator", 
    origin: "London, c. 1885", 
    category: "electronics-collectibles", 
    ...
    description: "Intricately engraved brass astronomical instrument used for determining the positions of celestial bodies." 
  },
  { 
    id: 3, 
    title: "Vintage Solid Gold Pocket Watch", 
    origin: "Geneva, c. 1910", 
    category: "antiques", 
    image: "https://images.unsplash.com/photo-1522337360788-8b13edd793be?w=600", 
    images: ["https://images.unsplash.com/photo-1522337360788-8b13edd793be?w=600"], 
    basePrice: 8500, 
    currentBid: 9200, 
    bidsCount: 16, 
    estimate: "$10,000 - $14,000", 
    description: "Engine-turned 18k yellow gold case containing an pristine, hand-wound Swiss movement." 
  }
];

// Exact Collectibles categories from your marketplace image (image_96b9e8.png)
const SUB_CATEGORIES = [
  { id: 'all', name: 'View All' },
  { id: 'advertising-paper-ephemera', name: 'Advertising, Paper & Ephemera' },
  { id: 'animation-art', name: 'Animation Art' },
  { id: 'antiques', name: 'Antiques' },
  { id: 'autographs', name: 'Autographs' },
  { id: 'books-maps-manuscripts', name: 'Books, Maps & Manuscripts' },
  { id: 'coins-money-stamps', name: 'Coins, Money & Stamps' },
  { id: 'couture-fashion-accessories', name: 'Couture, Fashion & Accessories' },
  { id: 'electronics-collectibles', name: 'Electronics Collectibles' },
  { id: 'historical-political-space-collectibles', name: 'Historical, Political & Space Collectibles' },
  { id: 'memorabilia', name: 'Memorabilia' },
  { id: 'military-wartime-collectibles', name: 'Military & Wartime Collectibles' }
];
...
export default function Collectibles() {
  ...
  const filteredItems = selectedCategory === 'all'
    ? COLLECTIBLES_ITEMS
    : COLLECTIBLES_ITEMS.filter(item => item.category === selectedCategory);
  ...
  const handlePrevItem = () => {
    const currentIndex = COLLECTIBLES_ITEMS.findIndex(i => i.id === selectedItem.id);
    const prevIndex = currentIndex === 0 ? COLLECTIBLES_ITEMS.length - 1 : currentIndex - 1;
    openModal(COLLECTIBLES_ITEMS[prevIndex]);
  };

  const handleNextItem = () => {
    const currentIndex = COLLECTIBLES_ITEMS.findIndex(i => i.id === selectedItem.id);
    const nextIndex = currentIndex === COLLECTIBLES_ITEMS.length - 1 ? 0 : currentIndex + 1;
    openModal(COLLECTIBLES_ITEMS[nextIndex]);
  };
  ...
  // Spot-on precise magnifier calculations
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-12 py-16 md:py-24">
      ...
      <div className="text-[10px] uppercase tracking-wider text-neutral-400 mb-6 font-sans">
        Home &gt; <span className="text-neutral-900 font-medium">Collectibles</span>
      </div>
            ...
            <span>
              {currentCategoryName} ({
                selectedCategory === 'all'
                  ? COLLECTIBLES_ITEMS.length
                  : COLLECTIBLES_ITEMS.filter(item => item.category === selectedCategory).length
              })
            </span>
            ...
            <div className="absolute left-0 right-0 mt-1 bg-white border border-neutral-200 shadow-xl max-h-60 overflow-y-auto z-40">
              {SUB_CATEGORIES.map((cat) => {
                const count = cat.id === 'all'
                  ? COLLECTIBLES_ITEMS.length
                  : COLLECTIBLES_ITEMS.filter(item => item.category === cat.id).length;
        ...
        <main className="w-full lg:w-3/4">
          <div className="border-b border-neutral-100 pb-4 mb-6 md:mb-8">
            <h1 className="font-serif text-2xl sm:text-3xl lg:text-4xl text-neutral-900 capitalize">
              {selectedCategory === 'all' ? 'Explore Collectibles' : selectedCategory.replace(/-/g, ' ')}
            </h1>
            <p className="text-xs lg:text-[13px] text-neutral-400 font-sans mt-1">
              Discover historical rarities and pop culture treasures.
            </p>
          </div>
          ...
          <div className="bg-white w-full max-w-6xl rounded-none relative shadow-2xl flex flex-col lg:flex-row border border-neutral-200 max-h-[90vh] lg:max-h-none overflow-hidden my-auto">
            ...
            <div className="w-full lg:w-[55%] p-4 sm:p-6 lg:p-8 flex flex-col justify-between bg-neutral-50 border-b lg:border-b-0 lg:border-r border-neutral-200 relative overflow-y-auto">
              ...
              <div className="mt-4 flex items-center justify-center space-x-1.5 text-neutral-400 font-sans text-[10px] uppercase tracking-wider">
                ...
                <span>Hover item to inspect stamp qualities, hallmarks & serial markings</span>
              </div>
            </div>
              ...
              >
                <div className="bg-[#8B1E2F] text-white px-4 py-3 flex items-center justify-between text-[10px] font-sans tracking-widest uppercase font-bold">
                  <span className="flex items-center">
                    ...
                    Detail Authentication Inspection
                  </span>
              ...
              <div className="space-y-4 lg:space-y-5">
                ...
                <div>
                  ...
                  <p className="font-sans text-xs lg:text-sm italic text-neutral-500 mt-1">
                    Provenance/Origin: <span className="font-semibold text-neutral-800 not-italic font-sans">{selectedItem.origin}</span>
                  </p>
                </div>
                ...
                <div className="space-y-1.5 pt-1">
                  ...
                  <ul className="grid grid-cols-2 gap-x-3 gap-y-1 text-[10px] text-neutral-500 font-sans font-medium pl-1">
                    <li className="flex items-center space-x-1">
                      <span className="text-emerald-500">✓</span> <span>Authenticity Insured</span>
                    </li>
                    <li className="flex items-center space-x-1">
                      <span className="text-emerald-500">✓</span> <span>Premium Grading Verified</span>
                    </li>
                  </ul>
                </div>
