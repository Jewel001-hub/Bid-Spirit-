...
export default function Hero() {
  ...
  // Added dynamic 'link' properties matching your routes
  const slides = [
    {
      image: lionArt,
  ...
  return (
    <div className="relative w-full h-screen bg-neutral-900 overflow-hidden select-none">
      ...
      {/* Added 'pointer-events-none' to prevent full-screen container from swallowing mobile touches */}
      <div className="absolute inset-0 z-10 flex flex-col justify-center items-start px-6 md:px-12 lg:px-24 max-w-3xl text-white pointer-events-none">
        
        {/* Restored 'pointer-events-auto' so only the active text and buttons register taps/clicks */}
        <div key={currentSlide} className="space-y-4 animate-fade-in-up pointer-events-auto">
          ...
          <div className="pt-4">
            {/* Swapped to <Link> for instant, clean SPA navigation with cursor-pointer */}
            <Link 
              to={slides[currentSlide].link}
              className="inline-block group relative px-8 py-3.5 border border-white overflow-hidden rounded-full transition-colors duration-500 cursor-pointer"
            >
              <span className="relative z-10 font-sans sm:text-xs md:text-lg lg:text-xs font-semibold uppercase tracking-widest text-white transition-colors duration-300">
                View Lot Provenance
              </span>

              {/* Slide-in colored accent background */}
