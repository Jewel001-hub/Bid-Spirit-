  ...
  const slides = [
    {
      ...
      title: "The Lion & The Lamb Study",
      desc: "An exceptional, heavily-textured oil on canvas highlighting raw heritage, timeless contrast, and unparalleled depth.",
    },
    {
      image: watchAsset,
    ...
    <div className="relative w-full h-screen bg-neutral-900 overflow-hidden select-none">
      ...
      {slides.map((slide, index) => (
        ...
        >
          ...
          <div className="absolute inset-0 bg-black/50 backdrop-gradient" />
        </div>
      ))}
      ...
      <div className="absolute inset-0 z-10 flex flex-col justify-center items-start px-6 md:px-12 lg:px-24 max-w-3xl text-white">
        ...
        <div key={currentSlide} className="space-y-4 animate-fade-in-up">
          <span className="font-sans text-xs font-bold uppercase tracking-[0.3em] text-[#8B1E2F] block">
            {slides[currentSlide].tag}
          </span>
          <h1 className="font-serif text-4xl md:text-6xl font-normal leading-tight tracking-wide">
            {slides[currentSlide].title}
          </h1>
          <p className="font-sans text-sm md:text-base text-neutral-300 font-light max-w-lg leading-relaxed">
          ...
          <div className="pt-4">
            <button className="group relative px-8 py-3.5 border border-white overflow-hidden rounded-full transition-colors duration-500">
              <span className="relative z-10 font-sans text-xs font-semibold uppercase tracking-widest text-white group-hover:text-white transition-colors duration-300">
                View Lot Provenance
              </span>
        ...
        {slides.map((_, index) => (
          <button
            ...
            className={`h-1.5 transition-all duration-500 rounded-full ${
              index === currentSlide ? "w-8 bg-[#8B1E2F]" : "w-2 bg-white/40 hover:bg-white"
            }`}
          />
        ))}
      </div>
